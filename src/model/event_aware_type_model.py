...
from .constrained_gen_type import BartConstrainedGenType
import torch.nn as nn

logger = logging.getLogger(__name__)

class GenIETypeModel(pl.LightningModule):
    def __init__(self, args):
        ...
        self.model = BartConstrainedGenType(self.config, self.tokenizer)
    ...
    def training_step(self, batch, batch_idx):
        ...
        inputs = {
                    ...
                    # "token_type": torch.zeros_like(batch["input_token_ids"]), 
        ...
        #     input_token_ids = batch["input_token_ids"][batch['compare']]
        #     input_attn_mask = batch["input_attn_mask"][batch['compare']]
        #     decoder_input_ids = batch['tgt_token_ids'][batch['compare']]
        #     decoder_attention_mask = batch['tgt_attn_mask'][batch['compare']]
        #     token_type = batch['token_type'][batch['compare']]
        #     compare_mask = batch['compare_mask'][batch['compare']]
        #     inputs = {
        ...
        #     argument_hidden_state_1 = encoder_last_hidden_state[compare_mask]
        #     argument_hidden_state_2 = encoder_last_hidden_state_compare[compare_mask]
    ...
    def test_step(self, batch, batch_idx):
        if self.hparams.sample_gen:
            ...
            # sample_output = self.model.generate(batch['input_token_ids'], token_type = torch.zeros_like(batch['input_token_ids']), do_sample=False, 
                            #     max_length=30, num_return_sequences=1,num_beams=1,
                            # )
            # print(batch['token_type'])
            sample_output = self.model.generate(batch['input_token_ids'], token_type = batch['token_type'], do_sample=False, 
                                max_length=30, num_return_sequences=1,num_beams=1,
                            )
