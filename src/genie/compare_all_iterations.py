...
def scorer(args):
    ...
    def get_examples(gen_file):
        examples = {}
        doc2ex = defaultdict(list) # a document contains multiple events 
        with open(gen_file,'r') as gen:
            for lidx, gen_line  in enumerate(gen): # this solution relies on keeping the exact same order 
    ...
    for iter_idx in range(6):
        pred_file = f'{args.prediction_dir}{iter_idx}_predictions.jsonl'
        examples = get_examples(pred_file)
        for event_idx in tqdm(examples):
            score = sum(get_score_for_one_event(examples[event_idx]))/4
            iter_dict[iter_idx].append(score)
    
    print('id\t0\t1\t2\t3\t4\t5')

    for event_id in range(len(iter_dict[0])):
        score_list = [iter_dict[iter][event_id] for iter in range(6)]
        if max(score_list[:3]) <= score_list[3]:
            continue
        scores = '\t'.join('{:.2}'.format(score) for score in score_list)
        print(f'{event_id}\t{scores}\tbest_iter: {score_list.index(max(score_list))}')
        
    # def get_score_for_one_event(event):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--prediction_dir',type=str,default=f'checkpoints/debug_iter/epoch_5_test_step_' )
