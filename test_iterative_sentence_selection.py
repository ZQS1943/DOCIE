...
from src.data.get_data import get_data_tag_only_sentence_selection, get_data_normal_sentence_selection
...
def main():
    ...
    # target = f'{args.data_file}/test.json'
    # coref_file = 'data/wikievents/coref/test.jsonlines'
    # get_data_normal_sentence_selection(source = source, target = target, tokenizer = tokenizer, coref = coref_file)
    # eval_dataset = IEDataset(target)
    eval_dataset = IEDataset('./preprocessed_sentence_selection_info/test_info.jsonl')    
    ...
    for iter_step in range(args.num_iterative_epochs):
        ...
        get_data_tag_only_sentence_selection(source = data_dir, target = target, tokenizer = tokenizer, coref = coref_file)
