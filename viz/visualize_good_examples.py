...
if __name__ == "__main__":
    ...
    parser.add_argument('--result-file',type=str, default='good_cases_from_baseline_to_best.json')
    args = parser.parse_args() 

    render_dicts = [] 

    with open(args.result_file, 'r') as f:
        good_examples = json.loads(f.read())
    
    for case_t in good_examples:
        for idx,case in enumerate(case_t):
            context_words = case['input_tokens']
            render_dict = {
                    "text":' '.join(context_words),
                    "ents": [],
                    "title": f"Event ID: {case['event_id']} {'bad' if idx else 'good'}",
                    "bg": [],
                }
